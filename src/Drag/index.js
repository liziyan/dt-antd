 ...
 * />
 * 参数说明：
 * @params[value]:要显示的数据，必填
 *   content: 展示的内容，object格式，必填
 *   code： 这个字段作为拖拽元素的key，必须唯一
 *   sort： 这个字段作为排序，int数字，排队目前只支持从小到大
 * @params[style]：其它样式,非必填
 * @params[onChange]：拖拽后的value数据，必填
 * @params[sortKey]：排序的对应字段，示例中对应的是value中的sort，默认sort，选填
 * @params[codeKey]：拖拽元素的对应字段，示例中对应的是value中的code，默认code，选填
 *   
 */
...
export default class VDraggable extends Component {
    ...
    render() {
      let sortKey = this.props.sortKey || 'sort';
      let codeKey = this.props.codeKey || 'code';
        const { value, style} = this.props;
        const { uId } = this.state;
        return (
            <div>
                <div style={{ display: 'flex', flexDirection: 'row', flexWrap: 'wrap' }}>
                    {this.createDraggleComponent(value, sortKey, style, uId, codeKey)}
                </div>
            </div>
        )
    }
